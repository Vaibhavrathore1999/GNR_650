...
def get_datasets(dataset, transform, root_dir='./data'):
    ...
    if dataset == "food101":
        ...
        reshuffled_labels = df.iloc[:, -1].to_numpy() 
        reshuffled_images = df.iloc[:, :-1].to_numpy()  
        reshuffled_images = reshuffled_images.reshape(-1, 32, 32, 3)
        train_dataset.data = reshuffled_images.astype(np.uint8) 
        train_dataset.targets = reshuffled_labels.tolist()  
        
        noise = np.array(train_dataset.targets)
        np.save("/raid/speech/soumen/gnr/IITB-GNR650-ADLCV/Assignment1/data/labels/cifar100_train_noise.npy", noise)
        print("Dataset reshuffled order has been successfully loaded into CIFAR-100.")
